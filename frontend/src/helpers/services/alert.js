...
import {
    TIMEOUT_TO_HIDE_SUCCESS_NOTIFICATION,
    TIMEOUT_TO_HIDE_ERROR_NOTIFICATION,
...
export const showAlert = ({type = 'info', title = '', text, onClose, timeout}) => {
    ...
    case 'error':
        ReactSAlert.error(text, {
            customFields: {title: title.toUpperCase(), className: 'error'},
    ...
    case 'success':
        ReactSAlert.success(text, {
            customFields: {title: title.toUpperCase(), className: 'success'},
